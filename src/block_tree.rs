...
/// The ledger of all transactions, which everyone in the Coolcoin network accepts as the
...
/// However, this is out of scope for now, so we are going to use the height as a proxy to represent
/// the total work.
...
impl BlockTree {
    ...
    /// Returns a copy of all the blocks in the block tree with no particular order.
